...
describe('Cloudflare Worker Analytics Helper Functions', () => {
    ...
    it('keeps requested analytics periods within the supported range', () => {
        expect(getRequestedDays('-12')).toBe(1);
        expect(getRequestedDays('999')).toBe(365);
        expect(getRequestedDays('invalid')).toBe(30);
    });

    it('does not grant CORS access to untrusted origins', () => {
        const req = new Request('https://mezz.dev/api/visits', {
            headers: { Origin: 'https://untrusted.example' },
        });
        expect(getCorsHeaders(req)['Access-Control-Allow-Origin']).toBeUndefined();
    });
});
...
describe('Cloudflare Worker Fetch Handler', () => {
    ...
    it('does not trust client-provided IP or geo data', async () => {
        ...
        expect(data.visit.city).toBe('Lyon');
        expect(data.visit.country).toBe('FR');
        expect(data.visit.created_at).not.toBe('2000-01-01T00:00:00.000Z');
    });
